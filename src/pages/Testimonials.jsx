...
const Testimonials = () => {
    return (
        ...
        viewport={{ once: true}}
        id='Testimonials' className='w-full flex flex-col items-center pt-20 gap-15 px-6 md:px-20 lg:px-32'>
            <Title
                heading="Customer"
            ...
            <div className='flex justify-center flex-wrap gap-8 ' >
                {testimonialsData.map((testimonial, index) => (
                    <div key={index} className='max-w-[360px] px-8 py-10 md:px-10 md:py-12 gap-4 mb-8 border border-gray-200 shadow-md rounded-sm text-center'>
                        <img src={testimonial.image} alt={testimonial.alt} className='mb-4 mx-auto' />
                        ...
                        <p className='text-gray-700 text-md md:text-lg leading-tighter'>{testimonial.text}</p>
                    </div>
                ))}
            </div>
        </motion.div>
